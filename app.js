...
if (savedSession) {
    ...
    // Small delay to let the HTML load before switching screens
    setTimeout(() => {
        loadDashboard();
    }, 100);
}
...
function changePassword() {
    ...
    db.collection("users").doc(currentUser.id).update({ password: newPass }).then(() => {
        ...
        // Update the saved session password too!
        localStorage.setItem('ifa_user', JSON.stringify(currentUser));
...
function logout() {
    currentUser = null;
    // WIPE SESSION FROM PHONE ON LOGOUT
